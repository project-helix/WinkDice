...
class APIRouter_Chat {
	...
	async sendChat(req: Req, res: Res) {
		...
		if (data.target === 'global') {
			...
			if ((await ValidateAuthenticity(data)) == false)
				...
				if (chat.muted.includes(data.sender) && !(chat.owner == data.sender))
					return addWScache({
						id: 'sendAimedMesg',
						args: {
							target: data.sender,
	...
	async getMessages(req: Req, res: Res) {
      ...
      try {
		...
		const FetchedMesgs = await FetchedMesgs_;
...
async function getChat(options) {
	return (await getStore(Chats, Caches.chatChannel, options)).data;
}
